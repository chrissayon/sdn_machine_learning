...
while counter < 120:
    ...
    t0 = time.time()
    # Send request
    response = requests.get('http://localhost:8080/stats/port/123917682137064')
    ...
    if response.status_code != 200:
        ...
        if len(data[dpid]) != 0:
            ...
            latest_rx_packets    = data[dpid][array_num]["rx_packets"]
            latest_tx_packets    = data[dpid][array_num]["tx_packets"]
            latest_rx_bytes      = data[dpid][array_num]["rx_bytes"]
            latest_tx_bytes      = data[dpid][array_num]["tx_bytes"]
            latest_rx_dropped    = data[dpid][array_num]["rx_dropped"]
            latest_tx_dropped    = data[dpid][array_num]["tx_dropped"]
            latest_rx_errors     = data[dpid][array_num]["rx_errors"]
            latest_tx_errors     = data[dpid][array_num]["tx_errors"]
            latest_rx_frame_err  = data[dpid][array_num]["rx_frame_err"]
            latest_rx_over_err   = data[dpid][array_num]["rx_over_err"]
            latest_rx_crc_err    = data[dpid][array_num]["rx_crc_err"]
            latest_collisions    = data[dpid][array_num]["collisions"]
            latest_duration_sec  = data[dpid][array_num]["duration_sec"]
            latest_duration_nsec = data[dpid][array_num]["duration_nsec"]
            ...
            csv_open.writerow([data[dpid][array_num]["port_no"],
                                diff_rx_packets,
                                diff_tx_packets,
                                diff_rx_bytes,
                                diff_tx_bytes,
